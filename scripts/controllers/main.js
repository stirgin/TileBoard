App.controller('TileBoardMainController', [
    '$scope',
    '$location',
    TileBoardMainController
]);
...
function loadConfig(configName) {
    return new Promise(function(resolve, reject) {
        ...
        xhr.open("GET", `${DOMAIN}/configs/${configName}.js`);

        xhr.onload = function() {
            if (xhr.status === 200) {
                const config = eval(xhr.responseText);
...
//     return !!~location.pathname.search(/common$/);
// }

function isCommonPage($location) {
    return !!~$location.$$path.search(/common$/);
}

function TileBoardMainController($scope, $location) {
    // const url = new URL(location);
    // const scriptName = url.searchParams.get('config');
    const configName = $location.$$search['config'];

    if (configName) {
        loadConfig(configName)
            .then(function(config) {
                console.log(`Config was loaded: ${configName}`);
                configWasLoaded($scope, $location, config);
            }, function() {
                console.log(`Config was not loaded: ${configName}`);
                configWasLoaded($scope, $location, {});
            })
            .catch(function(err) {
                let errorMessage = err.hasOwnProperty('request') ?
    ...
    } else if (isCommonPage($location)) {
         configWasLoaded($scope, $location, window.COMMON);
    } else {
        console.log('Config was missed in url');
    }
}



function configWasLoaded($scope, $location, newConfig) {
